...
const LoginForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Login function now handles navigation internally
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-[#020617] rounded-2xl p-8 border border-slate-200 dark:border-slate-700 shadow-2xl shadow-blue-500/10">
        <h2 className="text-3xl font-bold text-center mb-8 text-[#051df5]">Welcome Back</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full rounded-xl bg-linear-to-r from-indigo-500 to-cyan-400 px-6 py-4 text-lg font-semibold text-white shadow-md hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center"
          >
            {isLoading ? (
              <>
        ...
        <div className="mt-6 text-center text-slate-900 dark:text-slate-300 text-sm">
          <p>
            ...
            <a href="/register" className="text-[#051df5] dark:text-indigo-400 font-bold hover:text-[#051df5]/80 dark:hover:text-indigo-300 transition-colors">
              Sign up
            </a>
          </p>
        </div>
